...
class HomeWidget(ScriptedLoadableModuleWidget, VTKObservationMixin):
    ...
    def setSlicerUIVisible(self, visible):
        ...
        keepToolbars.extend(
            [slicer.util.findChild(slicer.util.mainWindow(), f"{name}BottomToolBar") for name in ["Patients", "Planning", "Navigation", "Registration"]],
        )
...
class HomeLogic(ScriptedLoadableModuleLogic):
    ...
    def __init__(self, info: OpenNavUtils.Workflow, stack):
        ...
        for prevStep, nextStep in zip(self.steps[:-1], self.steps[1:], strict=False):
