...
def parameterProperty(
    ...
    if sum((factory is not MISSING, default is not MISSING)) > 1:
        raise ValueError(
            "More than one default option was provided. factory and default are mutually exclusive.",
        )
...
def nodeReferenceProperty(
    ...
    if sum((factory is not MISSING, default is not MISSING, class_ is not MISSING)) > 1:
        raise ValueError(
            "More than one default option was provided. factory, default, and class_ are mutually exclusive.",
        )
