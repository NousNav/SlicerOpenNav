...
class RegistrationWidget(ScriptedLoadableModuleWidget):
    ...
    def __init__(self, parent):
        ...
        self.workflow = OpenNavUtils.Workflow(
            "registration",
            nested=(
                OpenNavUtils.Workflow("patient-prep", setup=self.registrationStepPatientPrep, widget=self.ui.RegistrationStepPatientPrep),
                OpenNavUtils.Workflow("tracking-prep", setup=self.registrationStepTrackingPrep, widget=self.ui.RegistrationStepTrackingPrep, validate=self.trackerConnected),
                OpenNavUtils.Workflow("pointer-prep", setup=self.registrationStepPointerPrep, widget=self.ui.RegistrationStepPointerPrep, validate=self.trackerConnected),
                OpenNavUtils.Workflow("align-camera", setup=self.registrationStepAlignCamera, widget=self.ui.RegistrationStepAlignCamera, validate=self.trackerConnected),
                OpenNavUtils.Workflow("pivot-calibration", setup=self.registrationStepPivotCalibration, widget=self.ui.RegistrationStepPivotCalibration, validate=self.trackerConnected),
                OpenNavUtils.Workflow("spin-calibration", setup=self.registrationStepSpinCalibration, widget=self.ui.RegistrationStepSpinCalibration, validate=self.validateSpinCalibration),
                OpenNavUtils.Workflow("landmark-registration", setup=self.registrationStepLandmarkRegistration, widget=self.ui.RegistrationStepLandmarkRegistration, teardown=self.fiducialOnlyRegistration, validate=self.validateLandmarkRegistration),
                OpenNavUtils.Workflow("surface-registration", setup=self.registrationStepSurfaceRegistration, widget=self.ui.RegistrationStepSurfaceRegistration, validate=self.validateSurfaceRegistration, teardown=self.resetDefaultButtonActions),
                OpenNavUtils.Workflow("verify-registration", setup=self.registrationStepVerifyRegistration, validate=self.validateVerifyRegistration, widget=self.ui.RegistrationStepVerifyRegistration, teardown=self.registrationStepAcceptRegistration),
            ),
            setup=self.enter,
            teardown=self.exit,
        ...
        self.RMSE_SPIN_OK = 1.0
        self.RMSE_REGISTRATION_OK = 3.0
        self.RMSE_INITIAL_REGISTRATION_OK = 5.0
        self.RMSE_INITIAL_REGISTRATION_CONDITIONAL = 15.0
    ...
    def registrationStepLandmarkRegistration(self):
        ...
        self.AlignmentSideWidget.visible = True
        self.planningLogic.setPlanningNodesVisibility(skinModel=False, seedSegmentation=False, targetSegmentation=False, trajectory=False, landmarks=False)
    ...
    def registrationStepSurfaceRegistration(self):
        ...
        if not self.logic.landmark_registration_transform or OpenNavUtils.isLinearTransformNodeIdentity(self.logic.landmark_registration_transform) or not self.logic.landmark_registration_passed:
        ...
        self.planningLogic.setPlanningNodesVisibility(skinModel=True, seedSegmentation=False, targetSegmentation=False, trajectory=False, landmarks=False)
        OpenNavUtils.centerCam()
        ...
        if self.landmarks.landmarksFinished:
            ...
            if search is not None:
                ...
                # User can decided to proceed or not
                    ...
                    questionText = "Registration is poor (current RMSE: " + str(RMSE) + ", target RMSE: " + str(self.RMSE_INITIAL_REGISTRATION_OK) + "). Would you like to proceed anyway?"
    ...
    def startTracing(self):
        ...
        try:
            self.traceObserver = self.logic.pointer_to_headframe.AddObserver(slicer.vtkMRMLTransformNode.TransformModifiedEvent, self.doTracing)
        except:
            print("Warning:  tracker not connected")
    ...
    def stopTracing(self):
        ...
        if transformMatrix is not None:
            ...
            if self.logic.landmark_registration_transform:
                self.logic.landmark_registration_transform.SetAndObserveTransformNodeID(self.logic.surface_registration_transform.GetID())
                self.trace.traceNode.SetAndObserveTransformNodeID(self.logic.surface_registration_transform.GetID())
            else:
        ...
        self.logic.surface_registration_passed = (avg_dist_after - avg_dist_before < 0.0 - self.EPSILON) and (avg_dist_after < self.RMSE_REGISTRATION_OK)
    ...
    def computeTraceError(self):
        ...
        for i in range(trace_length):
            ...
            tracing_points[i, :] = p
            closest_point_on_surface = [0.0, 0.0, 0.0]
