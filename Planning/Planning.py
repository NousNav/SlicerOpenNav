    ...
    def __init__(self, parent):
        ...
        self.workflow = OpenNavUtils.Workflow(
            ...
            nested=(
                ...
                OpenNavUtils.Workflow("target", setup=self.planningStep2Target, teardown=self.logic.resetDefaultNodeAppearance, widget=self.ui.PlanningStep2, validate=self.validateTargetSegmentation),
                OpenNavUtils.Workflow("trajectory", setup=self.planningStep3Trajectory, teardown=self.logic.resetDefaultNodeAppearance, widget=self.ui.PlanningStep3, validate=self.validateTrajectorySegmentation),
                OpenNavUtils.Workflow("landmarks", setup=self.planningStep4Landmarks, teardown=self.logic.resetDefaultNodeAppearance, widget=self.ui.PlanningStep4, validate=self.validateDefineLandmarks),
            ),
            setup=self.enter,
            teardown=self.exit,
...
class PlanningLogic(ScriptedLoadableModuleLogic, VTKObservationMixin):
    ...
    def setupSkinSegmentationNode(self):
        if not self.skin_segmentation:
            ...
            skin_segment = segmentation.GetSegment(
    ...
    def setupSeedSegmentationNode(self):
        if not self.seed_segmentation:
            ...
            node.CreateDefaultDisplayNodes()
            segmentation = node.GetSegmentation()
            inside_segment = segmentation.GetSegment(
                segmentation.AddEmptySegment(  # noqa: F841
                    "OpenNav_INSIDE",
                    "OpenNav_INSIDE",
                    [0.10, 0.90, 0.10],
                )
            )
            outside_segment = segmentation.GetSegment(
                segmentation.AddEmptySegment(  # noqa: F841
                    "OpenNav_OUTSIDE",
                    "OpenNav_OUTSIDE",
                    [0.90, 0.10, 0.10],
